 ...
 */
package migration.service.v550.migrator;
...
public class OAuthDataMigrator extends Migrator {
    ...
    @Override public void migrate() throws MigrationClientException {
        try {
            addHashColumns();
    ...
    public void migrateOldEncryptedTokens() throws MigrationClientException, SQLException {
        ...
        try {
            log.info(Constant.MIGRATION_LOG
                    + "------------------Migration starting on OAuth2 access token table.------------------");
            ...
            try (Connection connection = getDataSource().getConnection()) {
                TokenDAO.getInstance().updateNewTokenHash(updatedOauthTokenList, connection);
            }
            log.info(Constant.MIGRATION_LOG + "------------------Migration completed on OAuth2 access token table"
                    + ".------------------");
        ...
        for (OauthTokenInfo oauthTokenInfo : oauthTokenList) {
            ...
            try {
                accessTokenHash = tokenPersistenceProcessor
                        .getProcessedAccessTokenIdentifier(oauthTokenInfo.getAccessToken());
                refreshTokenHash = tokenPersistenceProcessor.getProcessedRefreshToken(oauthTokenInfo.getRefreshToken());
    ...
    public void migrateAuthorizationCodes() throws MigrationClientException {
        ...
        try {
            log.info(Constant.MIGRATION_LOG + "------------------ Migration starting on OAuth2 authorization code "
                    + "table.------------------");
            ...
            log.info(Constant.MIGRATION_LOG
                    + "------------------Migration completed for  on OAuth2 authorization code table.------------------");
    ...
    private List<AuthzCodeInfo> transformAuthzCodeFromOldToNewEncryption(List<AuthzCodeInfo> authzCodeInfoList)
        ...
        for (AuthzCodeInfo authzCodeInfo : authzCodeInfoList) {
            ...
            String authCode = authzCodeInfo.getAuthorizationCode();
            authzCodeHash = tokenPersistenceProcessor.getProcessedAuthzCode(authCode);

            AuthzCodeInfo updatedAuthzCodeInfo = (new AuthzCodeInfo(authCode, authzCodeInfo.getCodeId()));
        ...
        try {
            try {
                if (!isClientSecretHashColumnsAvailable && OAuth2Util.isEncryptionWithTransformationEnabled()) {
                    log.info(Constant.MIGRATION_LOG
                            + "------------------Migration starting on OAuth2 consumer apps table.------------------");
